# Layn — Backstory Table of Contents

This directory contains the prose version of Layn's backstory, developed separately from [`notes.md`](notes.md).

`notes.md` remains the source of truth for story decisions, canon constraints, unresolved details, and planning.

The [character cheatsheet](cast.md) is the continuity reference for names, relationships, motives, voices, and status labels across the novel, audio, and comic adaptations.

The backstory is organized by **character → backstory → chapter**. Each chapter is a standalone prose file; audio renders and comic scripts remain in their respective production directories. Individual titles and scene boundaries may evolve as the prose is written.

## Story Structure

## Chapters 1–9

- **Chapter 1 — Menzoberranzan** — the city and its living architecture; the origins and nature of the drow.
- **Chapter 2 — The Web** — religion, rank, gender, and daily behavior.
- **Chapter 3 — House Freth** — house law, hierarchy, and the boy's final reveal.
- **Chapter 4 — A Normal Day** — Layn at ten, the Matron Mother, his older sisters, and Zilvrae's first lessons.
- **Chapter 5 — The Two Spiders** — the sisters' decision, the hidden signs, and the first spider ordeal.
- **Chapter 6 — The Small Corrections** — the nine-year bridge: the spider routine, sanctioned education, secrecy craft, phobia, hallucinations, and the path toward the hearing.
- **Chapter 7 — The Three Lessons** — the house-law lessons and the discoveries that reshape Layn's understanding of his place.
- **Chapter 8 — The Long Quiet** — the long middle period of training, fear, concealment, and change.
- **Chapter 9 — The Last Lesson** — the final assessment and spar with Zilvrae.
- **Chapter 10 — The Hearing** — the amplifier, the spirit arena, the Matron Mother's intervention, and the academy decision.

### Chapter 3 — The Academy/

The academy is a 10-year course (canon, Homeland p.151); the four sections below span Layn's first four years (≈1315–1318 DR). Melee-Magthere's next opening falls in the year Layn turns 19. Drizzt Do'Urden enters in Layn's third year (≈1317 DR) — a name and a technique, never a friend.

- **Section 1 — First Year** — the drow birthright of Dancing Lights, Darkness, and Faerie Fire
- **Section 2 — Second Year** — weapons, athletics, daggers, Sneak Attack, Savage Attacker, and Elven Accuracy
- **Section 3 — Third Year** — stealth, tools, perception, betrayal, spider trauma, and the first signs of Detect Magic. Drizzt Do'Urden arrives in the entering class — Layn hears of his technique and is amazed
- **Section 4 — Fourth Year** — Levitate, Dispel Magic, reliable Detect Magic, uncontrolled psionics — and the recall home: the shadow of Teken'duis reaches the academy, and House Freth summons its son

### Part II — The House That Eats Its Own

- **II.1 — Growing Up in Menzoberranzan**
  - II.1.1 — The City
  - II.1.2 — The Rules
  - II.1.3 — The Game
- **II.2 — The Boy Who Would Not Become What They Wanted**
  - II.2.1 — The Warrior
  - II.2.2 — The Psion
  - II.2.3 — The Doubt
- **II.3 — House Freth**
  - II.3.1 — The Family
  - II.3.2 — The Enemy
  - II.3.3 — The Shadow of Teken'Duis
- **II.4 — The Last Days**
  - II.4.1 — Before the Storm
  - II.4.2 — Something Is Wrong
  - II.4.3 — The Attack

### Part III — The Choice

- **III.1 — The Fall of House Freth**
  - III.1.1 — Chaos
  - III.1.2 — The Halls of the Matron
  - III.1.3 — The Choice
- **III.2 — The Dead and the Missing**
  - III.2.1 — The Final Clash
  - III.2.2 — The Disappearance
  - III.2.3 — Alone
- **III.3 — The Spider's Web**
  - III.3.1 — The Journey
  - III.3.2 — The Servants
  - III.3.3 — The Vision
- **III.4 — The Light**
  - III.4.1 — Awakening
  - III.4.2 — The Tunnel
  - III.4.3 — Luskan

### Postscript — The Web

The postscript follows the main ending and reveals the larger shape of Lolth's plan through a scene involving her servants.

## Narrative Approach

The story combines two lenses:

- **Layn's perspective:** an intimate view of his life and development as a young drow. His understanding is limited by his age and circumstances; he should not think with the knowledge of an outside observer.
- **Omniscient perspective:** a wider view of Menzoberranzan, drow society, politics, religion, customs, and history, including information Layn does not yet understand.

A complete subsection may follow another character—especially one of Layn's older sisters—when that viewpoint reveals motives or actions Layn cannot see. Viewpoint changes should happen at subsection boundaries rather than through abrupt head-hopping.

Worldbuilding should be embedded in lived scenes wherever possible rather than presented as encyclopedia-style exposition.

The prose should have its own voice. *Homeland* is a reference for immersive worldbuilding and narrative architecture, not a template for copying R. A. Salvatore's prose style.

The final prose should read naturally aloud: varied sentence length, clear paragraph rhythm, strong imagery, dialogue that is easy to follow when spoken, and enough breathing room for atmosphere and emotional beats.

## Drafting Status

**Chapters 1–9** are now drafted. I.2.2 has been revised against the final eight-scene plan while preserving the strongest material from the previous prose version; the current prose is the active version. **I.3 — The Academy** is outlined for the next drafting pass (four sections spanning the first four years of the 10-year course); Part II and Part III remain unchanged.
